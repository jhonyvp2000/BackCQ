...
// "ENFERMERO INSTRUMENTISTA" id from db
const ENFERMERO_INSTRUMENTISTA_ID = '33333333-3333-3333-3333-333333333333';
...
async function main() {
    ...
    const excelPath = path.join(process.cwd(), 'no_copiar _a_produccion', 'Licenciados.xlsx');
    const outPath = path.join(process.cwd(), 'no_copiar _a_produccion', 'Licenciados_No_Encontrados.xlsx');
    ...
    for (const row of data) {
        const fullName = row['Apellidos y nombres'];
        ...
        if (bestScore > 0.74) {
            if (bestMatches.length === 1) {
                ...
                await db.update(staffProfiles)
                    .set({ professionId: ENFERMERO_INSTRUMENTISTA_ID })
