  ...
  if (isTimeDefined === false) {
    ...
    return (
      <span className="flex items-center gap-1.5">
        {datePart}
        <span className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase shadow-sm">
          Por definir
        </span>
      </span>
    );
...
export const formatPatientDemographics = (
  ...
  const fullName =
    `${patientPii?.nombres || ""} ${patientPii?.apellidos || ""}`.trim();
  if (!fullName || fullName === "Desconocido")
    return <span className="text-zinc-500 font-normal">Desconocido</span>;
  ...
  const dob =
    ...
    patientPii?.fechaNacimiento ||
    patientPii?.fecha_nacimiento;
  if (dob) {
  ...
  const dni =
    patientPii?.dni ||
    patientPii?.carnetExtranjeria ||
    patientPii?.pasaporte ||
    patientPii?.numeroDocumento ||
    patient?.dni ||
    "";
...
export const formatDemographicsOnly = (
  patientPii: any,
  patient: any,
  bedNumber?: string | null,
) => {
...
export function SurgeryViewToggle({
  surgeriesData,
  salas,
  sortParams,
  specialties,
  ...
  const canEditTimes = permissions.includes(
    "editar_tiempos_fases:programacion",
  );
  ...
  const [transitionModal, setTransitionModal] = useState<{
    ...
    initialTime?: string;
    urgencyType?: string;
  }>({ isOpen: false, surgeryId: "", targetPhase: "", patientName: "" });
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("OPEN_TRANSITION_MODAL", handleOpenModal);
  }, []);
  ...
  const SortIcon = ({ columnKey }: { columnKey: string }) => {
    ...
    if (idx === -1)
      return (
        <ArrowUp
          size={12}
          className="text-zinc-300 dark:text-zinc-500 ml-1 opacity-0 group-hover:opacity-100 transition-opacity"
        />
      );
    ...
    return (
      <div className="flex items-center ml-1">
        {config.direction === "asc" ? (
          <ArrowUp
            size={12}
            className="text-[var(--color-hospital-blue)] opacity-100"
          />
        ) : (
          <ArrowDown
            size={12}
        ...
        {sortConfig.length > 0 && (
          <span
            className="text-[9px] font-bold text-[var(--color-hospital-blue)] ml-0.5 leading-none bg-blue-50 dark:bg-blue-900/30 px-1 py-[1px] rounded"
            title={`Prioridad ${idx + 1}`}
          >
            {idx + 1}
          </span>
        )}
  ...
  const baseFilteredSurgeries = surgeriesData.filter((s) => {
    if (filterPatient.trim() !== "") {
      const searchTerms = filterPatient
        .toLowerCase()
        .split(/\s+/)
        .filter(Boolean);
      const fullName =
        `${s.patient?.name || ""} ${s.patientPii?.nombres || ""} ${s.patientPii?.apellidos || ""}`.toLowerCase();
      ...
      for (const fs of filterStatus) {
        const isMissingData = (s: any) => {
          return (
            ...
            !s.surgery.surgeryType ||
            s.surgery.surgeryType.trim() === "" ||
            !s.surgery.urgencyType ||
            s.surgery.urgencyType.trim() === "" ||
            !s.surgery.specialtyId ||
            !s.surgery.origin ||
            s.surgery.origin.trim() === "" ||
            !s.surgery.requestDate ||
            !s.surgery.scheduledDate ||
            !s.surgery.operatingRoomId ||
            !s.surgery.estimatedDuration ||
            s.surgery.estimatedDuration.trim() === "" ||
            !s.team ||
            !s.team.some((t: any) => t.role === "ANESTESIOLOGO") ||
            !s.team.some((t: any) => t.role === "ENFERMERO") ||
            !s.surgery.anesthesiaType ||
            s.surgery.anesthesiaType.trim() === ""
          );
        };
  ...
  const filteredSurgeries = baseFilteredSurgeries.filter((s) => {
    if (
      filterRoom.length > 0 &&
      (!s.operatingRoom?.id || !filterRoom.includes(s.operatingRoom.id))
    ) {
      return false;
    }
    ...
    if (
      ...
      (!s.specialty?.id || !filterSpecialty.includes(s.specialty.id))
    ) {
      return false;
    }
    ...
    if (filterStaff.length > 0) {
      ...
      const hasMatchedStaff = filterStaff.some((staffId) =>
        s.team.some(
          (t: any) => t.staff?.id === staffId || t.staffId === staffId,
        ),
      );
  ...
  const compareByKey = (a: any, b: any, key: string) => {
    ...
    if (key === "hora") {
      const timeA = a.surgery.scheduledDate
        ? new Date(a.surgery.scheduledDate).getTime()
        : 0;
      const timeB = b.surgery.scheduledDate
        ? new Date(b.surgery.scheduledDate).getTime()
        : 0;
      return timeA - timeB;
    }
    if (key === "paciente") {
      const nameA =
        `${a.patientPii?.nombres || ""} ${a.patientPii?.apellidos || ""}`.trim();
      const nameB =
        `${b.patientPii?.nombres || ""} ${b.patientPii?.apellidos || ""}`.trim();
      return nameA.localeCompare(nameB);
    }
    if (key === "tipo") {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "scheduled":
        return (
          <span className="bg-blue-50 text-[var(--color-hospital-blue)] px-3 py-1.5 rounded-full text-xs font-bold border border-blue-200/50 flex flex-nowrap items-center gap-1.5 w-max shadow-sm">
      ...
      case "in_progress":
        return (
          <span className="bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-xs font-bold border border-amber-200/50 flex flex-nowrap items-center gap-1.5 w-max shadow-sm">
      ...
      case "anesthesia_start":
        return (
          <span className="bg-purple-50 text-purple-700 px-3 py-1.5 rounded-full text-xs font-bold border border-purple-200/50 flex flex-nowrap items-center gap-1.5 w-max shadow-sm">
      ...
      case "pre_incision":
        return (
          <span className="bg-fuchsia-50 text-fuchsia-700 px-3 py-1.5 rounded-full text-xs font-bold border border-fuchsia-200/50 flex flex-nowrap items-center gap-1.5 w-max shadow-sm">
      ...
      case "surgery_end":
        return (
          <span className="bg-cyan-50 text-cyan-700 px-3 py-1.5 rounded-full text-xs font-bold border border-cyan-200/50 flex flex-nowrap items-center gap-1.5 w-max shadow-sm">
      ...
      case "patient_exit":
        return (
          <span className="bg-orange-50 text-orange-700 px-3 py-1.5 rounded-full text-xs font-bold border border-orange-200/50 flex flex-nowrap items-center gap-1.5 w-max shadow-sm">
      ...
      case "urpa_exit":
        return (
          <span className="bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-full text-xs font-bold border border-indigo-200/50 flex flex-nowrap items-center gap-1.5 w-max shadow-sm">
      ...
      case "completed":
        return (
          <span className="bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-full text-xs font-bold border border-emerald-200/50 flex flex-nowrap items-center gap-1.5 w-max shadow-sm">
            <CheckCircle2 size={12} className="text-emerald-500" /> Finalizada
          </span>
        );
      case "cancelled":
        return (
          <span className="bg-red-50 text-red-700 px-3 py-1.5 rounded-full text-xs font-bold border border-red-200/50 flex flex-nowrap items-center gap-1.5 w-max shadow-sm">
            <XCircle size={12} className="text-red-500" /> Suspendida
          </span>
        );
      default:
        return (
          <span className="bg-zinc-100 text-zinc-600 px-3 py-1.5 rounded-full text-xs font-bold border border-zinc-200 flex flex-nowrap items-center gap-1.5 w-max shadow-sm">
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-4 left-1/2 -translate-x-1/2 z-[200] transition-all duration-300 pointer-events-none ${showSortLimitAlert ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}`}
      >
        <div className="bg-rose-50 dark:bg-rose-900/90 text-rose-700 dark:text-rose-100 px-5 py-3 rounded-2xl shadow-2xl border border-rose-200 dark:border-rose-800 flex items-center gap-3 backdrop-blur-md">
          ...
          <p className="text-sm font-bold tracking-tight">
            Solo se permite un máximo de 3 criterios de ordenamiento
            simultáneos. Quite un criterio para continuar.
          </p>
        </div>
      </div>
      ...
      <div className="px-6 py-5 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-900/20">
        ...
        <div className="flex items-center gap-4">
          <span className="bg-emerald-50 dark:bg-emerald-900/10 text-emerald-700 dark:text-emerald-400 px-3 py-1.5 rounded-xl text-xs font-bold border border-emerald-200/50 dark:border-emerald-800/50 shadow-sm hidden sm:block">
            {
              surgeriesData.filter((s) => s.surgery.status !== "cancelled")
                .length
            }{" "}
            Activas
          </span>
              ...
              {filteredSurgeries.length === 0 ? (
                <div className="p-12 text-center flex flex-col items-center justify-center flex-grow bg-zinc-50/30 dark:bg-zinc-900/10">
                  ...
                  <p className="text-zinc-500 dark:text-zinc-400 max-w-sm font-medium">
                    No hay intervenciones programadas para este filtro. Utiliza
                    el panel lateral para agendar la primera cirugía del día.
                  </p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 z-20">
                      <tr className="border-b border-zinc-300 dark:border-zinc-700">
                        ...
                        <th
                          scope="col"
                          className="px-3 py-2 text-xs font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-700 bg-zinc-50/95 dark:bg-zinc-800/95 backdrop-blur-md min-w-[70px] max-w-[100px]"
                        >
                          <div
                        ...
                        <th
                          scope="col"
                          className="px-3 py-2 text-xs font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-700 bg-zinc-50/95 dark:bg-zinc-800/95 backdrop-blur-md min-w-[35px] text-center"
                        >
                          <div
                        ...
                        <th
                          scope="col"
                          className="px-3 py-2 text-xs font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-700 bg-zinc-50/95 dark:bg-zinc-800/95 backdrop-blur-md min-w-[70px] text-center"
                        >
                          <div
                            className="flex items-center justify-center cursor-pointer group select-none"
                        ...
                        <th
                          scope="col"
                          className="px-3 py-2 text-xs font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-700 bg-zinc-50/95 dark:bg-zinc-800/95 backdrop-blur-md min-w-[130px] max-w-[180px]"
                        >
                          <div
                            className="flex items-center cursor-pointer group select-none"
                        ...
                        <th
                          scope="col"
                          className="hidden px-3 py-2 text-xs font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-700 bg-zinc-50/95 dark:bg-zinc-800/95 backdrop-blur-md min-w-[80px]"
                        >
                          <div
                            className="flex items-center cursor-pointer group select-none"
                        ...
                        <th
                          scope="col"
                          className="px-3 py-2 text-xs font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-700 bg-zinc-50/95 dark:bg-zinc-800/95 backdrop-blur-md min-w-[330px] max-w-[430px]"
                        >
                          Diagnóstico / Intervención
                        </th>
                        <th
                          scope="col"
                          className="px-3 py-2 text-xs font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-700 bg-zinc-50/95 dark:bg-zinc-800/95 backdrop-blur-md min-w-[160px]"
                        >
                          Equipo
                        </th>
                        <th
                          scope="col"
                          className="px-3 py-2 text-xs font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-700 bg-zinc-50/95 dark:bg-zinc-800/95 backdrop-blur-md min-w-[85px]"
                        >
                          Estado
                        </th>
                        <th
                          scope="col"
                          className="px-3 py-2 text-xs font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-700 bg-zinc-50/95 dark:bg-zinc-800/95 backdrop-blur-md min-w-[80px] text-center"
                        >
                          F. Sol-Pro
                        </th>
                        {!isListFullscreen && (
                          <th
                            scope="col"
                            className="px-3 py-2 pl-4 text-right text-xs font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-700 min-w-[80px] sticky right-0 z-30 bg-zinc-50/95 dark:bg-zinc-800/95 shadow-[-5px_0_10px_-5px_rgba(0,0,0,0.1)] before:content-[''] before:absolute before:inset-y-0 before:-left-[1px] before:w-[1px] before:bg-zinc-300 dark:before:bg-zinc-700"
                          >
                            Gestión
                          </th>
                        )}
                    ...
                    <tbody className="bg-white dark:bg-zinc-900 divide-y divide-zinc-300 dark:divide-zinc-700">
                      {sortedSurgeries.map((row, index) => {
                        const effectiveStatus =
                          optimisticStatuses[row.surgery.id] ||
                          row.surgery.status;
                        const isPending =
                          pendingStatuses[row.surgery.id] || false;

                        const logicalPhase = (() => {
                          if (row.surgery.status === "cancelled")
                            return "cancelled";
                          if (!row.surgery.actualStartTime) return "scheduled";
                          if (!row.surgery.anesthesiaStartTime)
                            return "in_progress";
                          if (!row.surgery.preIncisionTime)
                            return "anesthesia_start";
                          if (!row.surgery.surgeryEndTime)
                            return "pre_incision";
                          if (!row.surgery.patientExitTime)
                            return "surgery_end";
                          if (!row.surgery.urpaExitTime) return "patient_exit";
                          if (row.surgery.status === "completed")
                            return "completed";
                          return "urpa_exit";
                        })();
                        ...
                        if (effectiveStatus === "scheduled") {
                          pillClasses =
                            "bg-zinc-100 text-zinc-600 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-700 hover:bg-yellow-100 dark:hover:bg-yellow-900/50";
                          pillContent = (
                            <>
                          ...
                          pillClasses =
                            "bg-yellow-100 text-yellow-700 border-yellow-200 dark:bg-yellow-900/40 dark:text-yellow-400 dark:border-yellow-800/50 hover:bg-emerald-100 dark:hover:bg-emerald-900/50";
                          pillContent = (
                            <>
                          ...
                          pillClasses =
                            "bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-400 dark:border-emerald-800/50 hover:bg-red-100 dark:hover:bg-red-900/50";
                          pillContent = (
                            <>
                              <CheckCircle2
                                size={12}
                          ...
                          pillClasses =
                            "bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-400 dark:border-red-800/50 hover:bg-zinc-100 dark:hover:bg-zinc-800";
                          pillContent = (
                            <>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-1.5 align-middle max-w-[120px]">
                              <div
                                className="text-xs text-zinc-900 dark:text-zinc-100 font-bold line-clamp-3 leading-tight break-words whitespace-normal"
                                title={row.specialty?.name || ""}
                              >
                                {row.specialty?.name || "-"}
                              </div>
                            ...
                            <td className="px-3 py-1.5 whitespace-nowrap align-middle text-center">
                              <div className="flex items-start justify-center text-[13px] font-bold text-left">
                                <div
                                  className={`w-1.5 h-1.5 rounded-full mr-1.5 shrink-0 mt-1.5 ${row.operatingRoom?.name ? "bg-emerald-500" : "bg-red-500"}`}
                                ></div>
                                <span
                                  className="whitespace-normal break-words leading-tight max-w-[40px]"
                                  title={row.operatingRoom?.name || "S/A"}
                                >
                            ...
                            <td className="px-3 py-1.5 whitespace-nowrap align-middle text-center">
                              <div className="flex flex-col items-center justify-center gap-1.5">
                                <div className="text-[13px] font-bold text-amber-700 dark:text-amber-400">
                                  {row.surgery.isTimeDefined
                                    ? formatTimeOnly(row.surgery.scheduledDate)
                                    : "00:00"}
                                </div>
                            ...
                            <td className="px-3 py-1.5 align-middle max-w-[200px]">
                              <div className="text-xs whitespace-normal break-words leading-tight">
                                {row.patientPii?.dni === "00000000" ||
                                row.patientPii?.nombres === "POR DEFINIR" ? (
                                  <span className="font-bold text-zinc-500 italic uppercase tracking-wider">
                                    POR DEFINIR
                                  </span>
                                ) : (
                                  <>
                                    <span
                                      className="font-bold text-zinc-900 dark:text-white mr-1"
                                      title={
                                        row.patientPii?.nombres
                                          ? `${row.patientPii.nombres} ${row.patientPii.apellidos}`
                                          : "Desconocido"
                                      }
                                    >
                                      {row.patientPii?.nombres &&
                                      row.patientPii.nombres !== "Desconocido"
                                        ? `${row.patientPii.nombres} ${row.patientPii.apellidos}`
                                        : "Desconocido"}
                                    </span>
                                    <span className="text-[11px] text-zinc-900 dark:text-white font-mono tracking-tight mr-1">
                                      {row.patientPii?.dni ||
                                        row.patientPii?.carnetExtranjeria ||
                                        row.patientPii?.pasaporte ||
                                        "S/Doc"}
                                    </span>
                                    <span className="text-[11px] text-zinc-900 dark:text-white font-sans tracking-normal mr-1">
                                      {formatDemographicsOnly(
                                        row.patientPii,
                                        row.patient,
                                        row.surgery?.bedNumber,
                                      )}
                                    </span>
                            ...
                            <td className="px-3 py-1.5 align-middle min-w-[330px] max-w-[430px]">
                              <div className="flex flex-col gap-1 w-full">
                                ...
                                {row.procedures &&
                                  ...
                                  typeof procedures !== "undefined" && (
                                    <div
                                      className="text-[11px] text-purple-700 dark:text-purple-400 font-medium line-clamp-3 leading-tight break-words whitespace-normal"
                                      title={
                                        procedures.find(
                                          (pr) => pr.id === row.procedures[0],
                                        )?.name
                                      }
                                    >
                                      ...
                                      {procedures.find(
                                        (pr) => pr.id === row.procedures[0],
                                      )?.name || row.procedures[0]}{" "}
                                      {row.procedures.length > 1
                                        ? `(+${row.procedures.length - 1})`
                                        : ""}
                                    </div>
                                  )}
                                ...
                                {row.surgery.notes && (
                                  <div
                                    className="text-[10px] text-black dark:text-white line-clamp-2 leading-tight font-medium"
                                    title={row.surgery.notes}
                                  >
                                    {row.surgery.notes}
                                  </div>
                                )}
                                    ...
                                    {row.surgery.anesthesiaType
                                      ...
                                      .map((typeCode: string) => {
                                        ...
                                        return (
                                          <span
                                            key={trimmed}
                                            className="text-[9px] px-1.5 py-0.5 rounded-full border font-bold uppercase bg-sky-50 dark:bg-sky-950/30 text-sky-700 dark:text-sky-400 border-sky-200/60 dark:border-sky-800/60 shadow-sm inline-flex items-center leading-none"
                                          >
                                            Anestesia: {label}
                                          </span>
                                        );
                            ...
                            <td className="px-3 py-1.5 align-middle max-w-[250px]">
                              {row.team && row.team.length > 0 ? (
                                (() => {
                                  const surgeons = row.team.filter(
                                    (t: any) => t.role === "CIRUJANO",
                                  );
                                  const anesthesiologists = row.team.filter(
                                    (t: any) => t.role === "ANESTESIOLOGO",
                                  );

                                  const getRoleCode = (t: any) => {
                                    if (t.role === "INSTRUMENTISTA")
                                      return "In";
                                    if (t.role === "CIRCULANTE") return "Ci";
                                    let profName =
                                      t.staff?.professionName ||
                                      t.professionName;
                                    if (
                                      !profName &&
                                      ...
                                      const nurse = staff.nurses.find(
                                        (n: any) => n.id === t.staff.id,
                                      );
                                      if (nurse)
                                        profName = nurse.professionName;
                                    }
                                    if (profName?.includes("INSTRUMENTISTA"))
                                      return "In";
                                    if (profName?.includes("CIRCULANTE"))
                                      return "Ci";
                                    return "Other";
                                  };

                                  const instrumentistas = row.team.filter(
                                    (t: any) =>
                                      t.role !== "CIRUJANO" &&
                                      t.role !== "ANESTESIOLOGO" &&
                                      getRoleCode(t) === "In",
                                  );
                                  const circulantes = row.team.filter(
                                    (t: any) =>
                                      t.role !== "CIRUJANO" &&
                                      t.role !== "ANESTESIOLOGO" &&
                                      getRoleCode(t) === "Ci",
                                  );
                                  const others = row.team.filter(
                                    (t: any) =>
                                      t.role !== "CIRUJANO" &&
                                      t.role !== "ANESTESIOLOGO" &&
                                      getRoleCode(t) === "Other",
                                  ...
                                  const renderTeamGroup = (
                                    ...
                                    return (
                                      <div className="flex items-start gap-x-1">
                                        <span
                                          className={`font-bold ${colorClass} shrink-0`}
                                        >
                                          {prefix}:
                                        </span>
                                        <div className="flex flex-wrap gap-x-1.5">
                                          {members.map(
                                            (t: any, idx: number) => (
                                              <span
                                                key={`${row.surgery.id}-${t.staff.id}`}
                                                className="text-zinc-900 dark:text-zinc-100 font-bold"
                                                title={`${t.role}: ${t.staff.name} ${t.staff.lastname}`}
                                              >
                                                {t.staff.name?.split(" ")[0]}{" "}
                                                {
                                                  t.staff.lastname?.split(
                                                    " ",
                                                  )[0]
                                                }
                                  ...
                                  return (
                                    <div className="text-[11px] leading-none flex flex-col gap-0.5">
                                      {renderTeamGroup(
                                        surgeons,
                                        "Cx",
                            ...
                            <td className="px-3 py-1.5 whitespace-nowrap align-middle">
                              {canChangeStatus ? (
                                <button
                                  ...
                                  disabled={isPending}
                                  className={`px-2 py-0.5 rounded-full text-[10.5px] font-bold border flex flex-nowrap items-center justify-center gap-1.5 shadow-sm transition-all duration-300 cursor-pointer hover:scale-105 active:scale-95 ${pillClasses} ${isPending ? "opacity-50 scale-95" : ""}`}
                                ...
                                <div
                                  className={`px-2 py-0.5 rounded-full text-[10.5px] font-bold border flex flex-nowrap items-center justify-center gap-1.5 shadow-sm cursor-default opacity-90 ${pillClasses}`}
                            ...
                            {!isListFullscreen && (
                              <td className="px-3 py-1.5 whitespace-nowrap text-right align-middle sticky right-0 z-10 bg-white dark:bg-zinc-900 group-hover:bg-zinc-50/50 dark:group-hover:bg-zinc-800/50 transition-colors shadow-[-5px_0_10px_-5px_rgba(0,0,0,0.1)] before:content-[''] before:absolute before:inset-y-0 before:-left-[1px] before:w-[1px] before:bg-zinc-100 dark:before:bg-zinc-800/50">
                                <div className="flex justify-end gap-2 items-center opacity-70 group-hover:opacity-100 transition-opacity duration-300">
                                      ...
                                      <button
                                        onClick={() => {
                                          setTransitionModal({
                                            ...
                                            patientName:
                                              `${row.patientPii?.nombres || ""} ${row.patientPii?.apellidos || ""}`.trim() ||
                                              "Desconocido",
                                            urgencyType:
                                              row.surgery.urgencyType ||
                                              "ELECTIVO",
                                      ...
                                      <button
                                        onClick={() =>
                                          setTransitionModal({
                                            ...
                                            initialTime: formatForDateTimeLocal(
                                              row.surgery.actualStartTime,
                                            ),
                                            urgencyType:
                                              row.surgery.urgencyType ||
                                              "ELECTIVO",
                                      ...
                                      <button
                                        onClick={() =>
                                          setTransitionModal({
                                            ...
                                            patientName: `${row.patientPii?.nombres || ""} ${row.patientPii?.apellidos || ""}`,
                                            initialTime: formatForDateTimeLocal(
                                              row.surgery.anesthesiaStartTime,
                                            ),
                                            urgencyType:
                                              row.surgery.urgencyType ||
                                              "ELECTIVO",
                                  ...
                                  {logicalPhase === "scheduled" &&
                                    canCancel && (
                                      <button
                                        type="button"
                                        onClick={() => {
                                          setCancellingSurgery(row);
                                          setCancelConfirmText("");
                                        }}
                                        className="text-zinc-400 hover:text-amber-600 hover:bg-amber-50 p-2.5 rounded-xl transition-all"
                                  ...
                                  {logicalPhase === "completed" &&
                                    canViewReport && (
                                      <Link
                                        href={`/dashboard/programaciones/${row.surgery.id}/reporte`}
                                        className="inline-flex items-center text-zinc-500 hover:text-[var(--color-hospital-blue)] hover:bg-blue-50 p-2.5 rounded-xl transition-all tooltip bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700"
                                        title="Ver Reporte Operatorio"
                                      >
                                        <FileText size={18} />
                                      </Link>
                                    )}
              ...
              >
                <SurgeryTimeline
                  ...
                  salas={
                    filterRoom.length === 0
                      ? salas
                      : salas.filter((s) => filterRoom.includes(s.id))
                  }
