...
import { Loader2, PlusCircle, AlertCircle, CheckCircle, Search, Verified, Stethoscope, AlertTriangle } from "lucide-react";
import { importDiagnosesFromApi } from "@/app/actions/diagnoses";
import { lookupDiagnosisInApi } from "@/app/actions/cirugias";
import { motion, AnimatePresence } from "framer-motion";

export function CreateDiagnosisForm({ initialDiagnoses = [] }: { initialDiagnoses?: any[] }) {
    const [searchTerm, setSearchTerm] = useState("");
    const [isSearching, setIsSearching] = useState(false);
    const [localDiagnoses, setLocalDiagnoses] = useState<any[]>(initialDiagnoses);
    ...
    useEffect(() => {
        if (searchTerm.trim().length >= 3) {
            ...
            const timeoutId = setTimeout(async () => {
                try {
                    const resArray = await lookupDiagnosisInApi(searchTerm.trim());
    ...
    const filteredUnselectedDx = localDiagnoses
        .filter(dx => !selectedIds.has(dx.id))
        .filter(dx => {
    ...
    const selectedDxList = localDiagnoses.filter(dx => selectedIds.has(dx.id));
    ...
    const handleCreate = async () => {
        if (selectedIds.size === 0) {
            setActionMsg({ type: 'error', text: 'No has seleccionado ningún diagnóstico.' });
            return;
        }
        ...
        setActionMsg(null);
        try {
            const payload = selectedDxList.map(dx => ({
                id: dx.id,
                name: dx.name,
                code: dx.code
            }));

            const res = await importDiagnosesFromApi(payload);
            
            if (res.error) {
                setActionMsg({ type: 'error', text: res.error });
            } else {
                setActionMsg({ type: 'success', text: `${res.count} Registros empaquetados y validados con éxito.` });
        ...
        <div className="space-y-5">
            ...
            <div className="space-y-1.5">
                <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider flex items-center justify-between">
                    Motor PIDE - MINSA
                    {isSearching && <Loader2 size={12} className="animate-spin text-[var(--color-hospital-blue)]" />}
                </label>
                <div className="relative">
                    ...
                    <input
                        ...
                        placeholder="Buscar variables AND (Ej. apend K35.9)"
                        className="w-full pl-9 pr-4 py-2 text-sm rounded-xl border border-zinc-200 bg-white dark:bg-zinc-950 dark:border-zinc-800 text-zinc-900 dark:text-white focus:outline-none focus:border-[var(--color-hospital-blue)] focus:ring-1 focus:ring-[var(--color-hospital-blue)] transition-all shadow-sm"
                    />
                </div>
                {apiDown && (
            ...
            <div className={`mt-2 border rounded-xl overflow-hidden bg-white dark:bg-zinc-900 shadow-sm transition-all ${
                selectedIds.size > 0 ? "border-[var(--color-hospital-blue)] ring-2 ring-blue-500/20" : "border-zinc-200 dark:border-zinc-800"
            }`}>
                <div className="bg-zinc-50 dark:bg-zinc-800/50 px-3 py-2 text-xs font-bold uppercase tracking-wider text-zinc-500 border-b border-zinc-200 dark:border-zinc-800 flex justify-between">
                    ...
                    {selectedIds.size > 0 && <span className="text-[var(--color-hospital-blue)]">{selectedIds.size} Marcados</span>}
                </div>
                ...
                <div className="max-h-80 overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800/50">
                    ...
                    {selectedDxList.map((dx) => (
                        <label key={dx.id} className="flex items-start gap-3 p-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 cursor-pointer transition-colors group">
                            <input 
                                type="checkbox" 
                                checked={selectedIds.has(dx.id)}
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    ...
                                    {dx.id?.startsWith('__api') ? (
                                        <span className="flex items-center text-[10px] uppercase font-bold text-emerald-600 gap-0.5"><Verified size={10} /> MINSA API</span>
                                    ) : (
                    ...
                    {searchTerm && filteredUnselectedDx.length === 0 && selectedDxList.length === 0 && !isSearching && (
                        <div className="p-4 text-center text-xs text-zinc-500 font-medium flex flex-col items-center">
                            <Stethoscope size={24} className="text-zinc-300 mb-2" />
                            Ningún índice coincide con "{searchTerm}"
                        </div>
                    )}
                    {!searchTerm && selectedDxList.length === 0 && (
            ...
            <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800">
                <motion.button
                    ...
                    className="w-full relative flex items-center justify-center gap-2 rounded-xl bg-gradient-to-tr from-[var(--color-hospital-blue)] to-blue-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-blue-500/10 transition-all hover:shadow-blue-500/30 active:scale-95 disabled:opacity-70 disabled:pointer-events-none group overflow-hidden"
                >
                    ...
                    <span className="relative z-10">{isSubmitting ? 'Inyectando CIE-10...' : 'Compilar Diagnósticos y Guardar'}</span>
                </motion.button>
            </div>
        </div>
    );
