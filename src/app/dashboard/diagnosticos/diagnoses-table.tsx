...
export function DiagnosesTable({ records }: { records: any[] }) {
    ...
    const handleDelete = async () => {
        ...
        setActionMsg(null);
        try {
            const res = await deleteDiagnosis(deleteDx.id);
            if (res.error) setActionMsg({ type: 'error', text: res.error });
            else setDeleteDx(null);
        } catch (e: any) {
    ...
    const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const res = await updateDiagnosis(editDx.id, formData);
            if (res.error) setActionMsg({ type: 'error', text: res.error });
            else setEditDx(null);
        } catch (e: any) {
            setActionMsg({ type: 'error', text: e.message || 'Error de red.' });
        ...
        <div className="flex flex-col h-full bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800">
            ...
            <div className="sticky top-0 z-10 p-3 bg-zinc-50/80 dark:bg-zinc-800/80 backdrop-blur-xl border-b border-zinc-200 dark:border-zinc-800">
                <div className="relative max-w-lg mx-auto w-full">
                    ...
                    <input
                        type="search"
                        placeholder="Filtrar variables (Ej. aguda k35)"
                ...
                {filtered.length === 0 ? (
                    ...
                    <table className="w-full text-left text-sm text-zinc-600 dark:text-zinc-300">
                        <thead className="sticky top-0 bg-white dark:bg-zinc-900 shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] z-0 text-xs font-semibold uppercase tracking-wider text-zinc-500">
                            <tr>
                                <th className="px-5 py-4 w-28">Matrícula (CIE)</th>
                                <th className="px-5 py-4">Descriptor Patológico</th>
                        ...
                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/70">
                            {filtered.map((dx) => (
                                <tr key={dx.id} className="hover:bg-blue-50/40 dark:hover:bg-zinc-800/40 transition-colors group">
                                    <td className="px-5 py-3 font-mono text-zinc-900 dark:text-white font-bold bg-zinc-50/50 dark:bg-zinc-900/50">
                                        {dx.code ? (
                                            <span className="px-2 py-0.5 rounded border border-zinc-200 dark:border-zinc-700 text-xs tracking-widest">{dx.code}</span>
                                        ) : (
                                    ...
                                    <td className="px-5 py-3">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 text-indigo-500 flex items-center justify-center shadow-sm">
                                                <BookA size={14} />
                                            </div>
                                            <div className="font-medium text-zinc-900 dark:text-white leading-relaxed">{dx.name}</div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-5 py-3 text-right">
                                        <div className="flex justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button 
                                                onClick={() => { setEditDx(dx); setActionMsg(null); }}
                                            ...
                                            <button 
                                                onClick={() => { setDeleteDx(dx); setActionMsg(null); }}
                    ...
                    {deleteDx && (
                        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                            <motion.div
                                ...
                                onClick={() => !isSubmitting && setDeleteDx(null)}
                            />
                            <motion.div
                            ...
                            >
                                ...
                                <h3 className="text-lg font-bold text-center mb-2">Remover Patología</h3>
                                <p className="text-zinc-500 text-center text-sm leading-relaxed mb-6">
                                    Eliminarás el registro CIE <strong className="text-red-500">{deleteDx.code || 'S/C'}</strong> del catálogo maestro.
                                </p>
                    ...
                    {editDx && (
                        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                            <motion.div
                                ...
                                onClick={() => !isSubmitting && setEditDx(null)}
                            />
                            <motion.div
                            ...
                            >
                                ...
                                <h2 className="text-xl font-bold flex items-center gap-2 mb-6">
                                    <Edit2 className="text-indigo-500" size={20} /> Reclasificación Quirúrgica
                                </h2>
                                ...
                                <form onSubmit={handleEditSubmit} className="space-y-4">
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Código CIE-10</label>
                                        <input 
                                            name="code" 
                                            defaultValue={editDx.code || ''} 
                                            maxLength={10}
                                            className={`${getInputCls} uppercase font-mono text-indigo-600 dark:text-indigo-400 font-bold`} 
                                    ...
                                    <label className="flex items-center gap-3 pt-2">
                                        <input type="checkbox" name="isVerifiedMinsa" value="true" defaultChecked={editDx.isVerifiedMinsa} className="w-4 h-4 rounded border-zinc-300 text-[var(--color-hospital-blue)] focus:ring-[var(--color-hospital-blue)]" />
                                    ...
                                    <button 
                                        type="submit" 
                                        disabled={isSubmitting}
                                        className="w-full mt-4 flex items-center justify-center gap-2 bg-gradient-to-tr from-indigo-600 to-blue-600 text-white font-bold py-3 px-4 rounded-xl shadow-md hover:shadow-indigo-500/20 active:scale-95 transition-all disabled:opacity-70"
                                    >
